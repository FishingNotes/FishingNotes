...
fun DialogOnPlaceChoosing(
                ...
                mapView.getMapAsync { googleMap ->
                    ...
                    try {
                        ...
                        position?.first()?.let {
                            ...
                            } else viewModel.chosenPlace.value = unnamedPlaceStr
                        }
                    } catch (e: Throwable) {
                        viewModel.chosenPlace.value = cantRecognizePlace
                    }
                }
            }
        }
    ...
    val textColor by animateColorAsState(
        if (viewModel.chosenPlace.value != null) MaterialTheme.colors.onSurface
        else Color.LightGray
    )
...
fun PointerIcon(cameraMoveState: CameraMoveState, modifier: Modifier = Modifier) {
    ...
    val composition by rememberLottieComposition(LottieCompositionSpec.RawRes(
        if (darkTheme) R.raw.marker_night else R.raw.marker))
